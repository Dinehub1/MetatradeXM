...
class TradingSkill:
    ...
    def evaluate(self, symbol: str, direction: str, context: dict) -> dict:
        ...
        elif skill_type == "scaler":
            # Scaler skills are executed by position_scaler.py, not here.
            # Log a debug note so it's visible during skill evaluation.
            import logging as _log
            _log.getLogger(__name__).debug(
                f"Skill '{self.name}' type=scaler — delegation to position_scaler.py"
            )
...
{{"profitable": true/false, "suggestions": ["suggestion1", "suggestion2"],
  "param_changes": {{"key": "new_value"}}, "confidence": 0.0-1.0}}"""
        ...
        if result:
            ...
            return result
        log.warning(f"[SKILLS] Improve returned no data for '{skill_name}'")
...
{{"name": "skill-name", "description": "what it does",
  ...
  "body": "# Strategy\\n\\nDetailed rules here..."}}"""
        ...
        if not data:
            log.warning("[SKILLS] Auto-generate returned no data")
            return ""
